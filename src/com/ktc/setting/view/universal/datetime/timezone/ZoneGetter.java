...
public class ZoneGetter {
    ...
    public static final String KEY_LONG_DISPLAY_NAME = "long_name";
    private static final String XML_PATH = "/system/KtcConfig/TimeZone/timezones.xml";
    ...
    public static List<Map<String, Object>> getZonesList(Context context) {
        ...
        for (int i = 0; i < data.zoneCount; i++) {
            ...
            String longName = getZoneLongName(timeZoneNames, tz, now);
            Map<String, Object> displayEntry =
                    createDisplayEntry(tz, gmtOffsetText, displayName, offsetMillis, longName);
            zones.add(displayEntry);
        }
        zones.sort(new ZoneComparator());
        return zones;
    }

    private static class ZoneComparator implements Comparator<Map<String, Object>> {
        @Override
        public int compare(Map<String, Object> o1, Map<String, Object> o2) {
            return o1.get(KEY_GMT).toString().compareTo(o2.get(KEY_GMT).toString());
        }
    }

    private static Map<String, Object> createDisplayEntry(
            TimeZone tz, CharSequence gmtOffsetText, CharSequence displayName, int offsetMillis, String longName) {
        ...
        map.put(KEY_LONG_DISPLAY_NAME, longName);
        return map;
    }

    private static List<String> readTimezonesToDisplay() {
        ...
        try {
            ...
            while (parser.getEventType() != XmlResourceParser.END_TAG) {
                ...
                if (parser.getName().equals(XMLTAG_TIMEZONE)) {
                    String olsonId = parser.getAttributeValue(0);
    ...
    private static CharSequence getTimeZoneDisplayName(ZoneGetterData data,
        ...
        if (preferLongName) {
            ...
            if(tz.getID().equals("Europe/Istanbul")){
		    if(displayName ==null){
				displayName = timeZoneNames.getExemplarLocationName(tz.getID());
				}
			Log.d("ytt11","displayName111:"+displayName);
		} 
    ...
    public static final class ZoneGetterData {
        ...
        public ZoneGetterData(Context context) {
            ...
            final Date now = new Date();
            final List<String> olsonIdsToDisplayList = readTimezonesToDisplay();
            Log.d("dingyl", "olsonIdsToDisplayList : " + olsonIdsToDisplayList.size());
    ...
    private static String getGmtOffsetString(Context context, TimeZone tz) {
        List<Map<String, Object>> zoneList = getZonesList(context.getApplicationContext());
        for (Map<String, Object> zone : zoneList) {
            if (zone.get(KEY_ID).equals(tz.getID())) {
                return zone.get(KEY_OFFSET_LABEL).toString();
            }
        }
        return null;
    }

    private static String getZoneDisplayName(Context context, TimeZone tz) {
        List<Map<String, Object>> zoneList = getZonesList(context.getApplicationContext());
        for (Map<String, Object> zone : zoneList) {
            if (zone.get(KEY_ID).equals(tz.getID())) {
                return zone.get(KEY_DISPLAY_LABEL).toString();
            }
        }
        return null;
    }
