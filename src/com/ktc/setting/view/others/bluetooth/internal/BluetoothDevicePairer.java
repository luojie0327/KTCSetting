...
package com.ktc.setting.view.others.bluetooth.internal;
...
public class BluetoothDevicePairer {
    ...
    // Mouse: bonding -> connecting -> connected -> bonded
    //     STATUS_PAIRING -> nothing -> STATUS_CONNECTING -> STATUS_NONE -> nothing
    // Keyboard: bonding -> bonded -> connecting -> connected
    //     STATUS_PAIRING -> BOND_BONDED(onBonded ->onServiceConnected) -> STATUS_CONNECTING -> STATUS_NONE(onInputDeviceAdded)
    private BroadcastReceiver mInputConnectionStateReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            BluetoothDevice device = (BluetoothDevice) intent.getParcelableExtra(
                    BluetoothDevice.EXTRA_DEVICE);
            if (DEBUG) {
                Log.d(TAG, "There was a input connection status change for: " + device.getAddress());
            }

            /*if (device.equals(mTarget)) {
                int previousState = intent.getIntExtra(
                        BluetoothInputDevice.EXTRA_PREVIOUS_STATE,
                        BluetoothInputDevice.STATE_CONNECTING);
                int state = intent.getIntExtra(BluetoothInputDevice.EXTRA_STATE,
                        BluetoothInputDevice.STATE_CONNECTING);

                if (DEBUG) {
                    Log.d(TAG, "Input Connection states: old = " + previousState + ", new = " + state);
                }

                if (previousState == BluetoothInputDevice.STATE_CONNECTING) {
                    int deviceClass = mTarget.getBluetoothClass().getDeviceClass();
                    if (state == BluetoothInputDevice.STATE_CONNECTED) {
                        if (DEBUG) {
                            Log.d(TAG, "Input device connected");
                        }
                        if (deviceClass == BluetoothClass.Device.PERIPHERAL_POINTING) {  // Mouse device
                            setStatus(STATUS_CONNECTING);
                            Log.d(TAG, "This is a Mouse: STATUS_CONNECTING -> STATUS_NONE");
                            setStatus(STATUS_NONE);
                        } else {
                            // is keyboard device , do nothing
                        }
                    } else if (state == BluetoothInputDevice.STATE_DISCONNECTED) {
                        Log.d(TAG, "Failed to connect");
                        if (deviceClass == BluetoothClass.Device.PERIPHERAL_POINTING) {
                    ...
                    unregisterInputConnectionStateReceiver();
                }
            }*/
        }
    };
    ...
    // MStar Android Patch Begin
    private boolean mInputConnectionStateReceiverRegistered = false;
    // MStar Android Patch End
    ...
    public void dispose() {
        ...
        // MStar Android Patch Begin
        if (mInputConnectionStateReceiverRegistered) {
            unregisterInputConnectionStateReceiver();
        }
        // MStar Android Patch End
    ...
    private void startBonding() {
        ...
        if (mTarget.getBondState() != BluetoothDevice.BOND_BONDED) {
            ...
            // MStar Android Patch Begin
            registerInputConnectionStateReceiver();
            // MStar Android Patch End
    ...
    private void openConnection() {
        ...
        int deviceClass = mTarget.getBluetoothClass().getDeviceClass();
        if (deviceClass == BluetoothClass.Device.PERIPHERAL_POINTING) { // Mouse device
            Log.d(TAG, "openConnection ==> This is a Mouse");
            // nothing to do
        } else {
    ...
    // MStar Android Patch Begin
    private void registerInputConnectionStateReceiver() {
        if (DEBUG) Log.d(TAG, "registerInputConnectionStateReceiver()");
        /*IntentFilter filter = new IntentFilter(BluetoothInputDevice.ACTION_CONNECTION_STATE_CHANGED);
        mContext.registerReceiver(mInputConnectionStateReceiver, filter);
        mInputConnectionStateReceiverRegistered = true;*/
    }

    private void unregisterInputConnectionStateReceiver() {
        if (mInputConnectionStateReceiverRegistered) {
            if (DEBUG) Log.d(TAG, "unregisterInputConnectionStateReceiver()");
            mContext.unregisterReceiver(mInputConnectionStateReceiver);
            mInputConnectionStateReceiverRegistered = false;
        }
    }
    // MStar Android Patch End
